...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const [leadStats, batchAndProgramStats, recentBatches] = await Promise.all([
      prisma.$queryRaw<Array<Record<string, bigint>>>`
        SELECT
          ...
          COUNT(*) FILTER (WHERE tier = 'filtered' AND match_status = 'no_match') AS "noMatchCount",
          COUNT(*) FILTER (WHERE tier = 'filtered' AND (match_status IS NULL OR match_status != 'no_match')) AS "unqualifiedCount",
    ...
    return successResponse({
      ...
      noMatchCount: Number(s.noMatchCount),
      unqualifiedCount: Number(s.unqualifiedCount),
