...
const menuItems: MenuItem[] = [
  ...
  { label: 'Credit Prescreen', href: '/admin/prescreen', icon: PrescreenIcon, roles: ['admin'] },
  ...
  { label: 'Quote Requests', href: '/admin/quotes', icon: QuotesIcon, roles: ['admin'] },
  { label: 'Loan Officers', href: '/admin/loan-officers', icon: LoanOfficersIcon, roles: ['admin'] },
  { label: 'Activity Log', href: '/admin/activity-log', icon: ActivityLogIcon, roles: ['admin'] },
...
function Sidebar() {
  ...
  // Load menu order from API — once on mount
  const menuLoadedRef = useRef(false);
  useEffect(() => {
    if (menuLoadedRef.current || visibleMenuItems.length === 0) return;
